  ...
  class Busser
    ...
    def setup_cmd
      @setup_cmd ||= if local_suite_files.empty?
        ...
        setup_cmd << %<#{sudo}sed -i -e "s/run(\\"PATH=.*bundle install\\")/run(\\"\#{Gem.bindir}\\/bundle install --local || \#{Gem.bindir}\\/bundle install\\")/" ${gem_bindir}/../gems/busser-rspec-*/lib/busser/runner_plugin/rspec.rb> if plugins.include? 'busser-rspec'
